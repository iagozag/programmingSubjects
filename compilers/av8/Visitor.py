...
class GenVisitor(Visitor):
    ...
    def visit_bln(self, exp, prog):
        ...
        prog.add_inst(AsmModule.Addi(v_name, "x0", value))
        return v_name
    ...
    def visit_eql(self, exp, prog):
        ...
        diff_name = self.next_var_name()
        prog.add_inst(AsmModule.Sub(diff_name, l_name, r_name))

        t1 = self.next_var_name()
        prog.add_inst(AsmModule.Slt(t1, diff_name, "x0")) 

        t2 = self.next_var_name()
        prog.add_inst(AsmModule.Slt(t2, "x0", diff_name))

        t3 = self.next_var_name()
        prog.add_inst(AsmModule.Add(t3, t1, t2))

        v_name = self.next_var_name()
        prog.add_inst(AsmModule.Xori(v_name, t3, 1))
    ...
    def visit_leq(self, exp, prog):
        ...
        r_name = exp.right.accept(self, prog)
        v_name = self.next_var_name()
        prog.add_inst(AsmModule.Slt(v_name, r_name, l_name))

        v2_name = self.next_var_name()
        prog.add_inst(AsmModule.Xori(v2_name, v_name, 1))
    ...
    def visit_neg(self, exp, prog):
        ...
        """
        # TODO: Implement this method.
        exp_name = exp.exp.accept(self, prog)
        v_name = self.next_var_name()
        prog.add_inst(AsmModule.Sub(v_name, "x0", exp_name))
        return v_name
    ...
    def visit_not(self, exp, prog):
        ...
        t1_name = self.next_var_name()
        prog.add_inst(AsmModule.Slt(t1_name, exp_name, "x0"))

        t2_name = self.next_var_name()
        prog.add_inst(AsmModule.Slt(t2_name, "x0", exp_name))

        t3_name = self.next_var_name()
        prog.add_inst(AsmModule.Add(t3_name, t1_name, t2_name))

        v_name = self.next_var_name()
        prog.add_inst(AsmModule.Xori(v_name, t3_name, 1))
    ...
    def visit_let(self, exp, prog):
        ...
        """
        # TODO: Implement this method.
        def_result_var = exp.exp_def.accept(self, prog)
        prog.add_inst(AsmModule.Addi(exp.identifier, def_result_var, 0))
