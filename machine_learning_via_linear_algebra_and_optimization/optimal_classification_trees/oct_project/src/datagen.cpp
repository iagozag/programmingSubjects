...
void GroundTruthTree::generate(int dtrue, int p_feat, int K_classes, std::mt19937& rng) {
    ...
    // forca folhas irmas a terem classes diferentes
...
int GroundTruthTree::classify(const std::vector<double>& x) const {
    ...
    while (v < first_leaf) {
        ...
        if (val >= split_threshold[mi]) v = 2 * v + 1;
        else                            v = 2 * v;
    }
...
void splitDataset(const Dataset& full, Dataset& train, Dataset& val, double train_ratio, std::mt19937& rng) {
    int n = full.n;
...
void saveDataset(const Dataset& ds, const std::string& filename) {
    ...
    if (!f) throw std::runtime_error("erro ao criar: " + filename);
